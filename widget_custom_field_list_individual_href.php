				...
				switch (http_request.readyState) {
					...
					case 4 : // COMPLETED
						if (http_request.status == 200) {
							...
							document.getElementById(cell_id).innerHTML = '<div class="error" style="vertical-align:middle; padding:1em; font-weight:normal;"><?php echo js_escape(__('There was a problem with the request. (Probably no data saved)','customfieldlist')); ?> ' + http_request.status + '<br />' + http_request.responseText + '</div>';
						}
						break;
	...
	if ('' != trim($opt['customfieldname'])) {
		...
		if ($nr_meta_values > 0) {
			...
			foreach ($meta_unique_values as $meta_id => $meta_value) {
				...
				echo '<div style="margin-top:0.5em;"><input name="customfieldlist_individual_href_meta_ids[]" type="hidden" value="'.strval($meta_id).'" />';
